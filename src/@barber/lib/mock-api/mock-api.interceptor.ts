...
import { BARBER_MOCK_API_DEFAULT_DELAY } from '@barber/lib/mock-api/mock-api.constants';
import { BarberMockApiService } from '@barber/lib/mock-api/mock-api.service';
import { delay, Observable, of, switchMap, throwError } from 'rxjs';

export const mockApiInterceptor = (request: HttpRequest<unknown>, next: HttpHandlerFn): Observable<HttpEvent<unknown>> =>
{
    const defaultDelay = inject(BARBER_MOCK_API_DEFAULT_DELAY);
    const barberMockApiService = inject(BarberMockApiService);

    // Try to get the request handler
    const {
        handler,
        urlParams,
    } = barberMockApiService.findHandler(request.method.toUpperCase(), request.url);
